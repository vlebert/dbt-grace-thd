...
- les **boîtiers** aux extrémités (`cb_bp1` / `cb_bp2`), résolus sur `elem_bp` (PBO, BPE **et** BPI) et enrichis de leur capacité via `cap_pbo` en `LEFT JOIN` (compteurs nuls hors PBO). Résoudre uniquement sur les PBO ferait basculer à tort les extrémités BPE/BPI en `source`/`target` `null`.
