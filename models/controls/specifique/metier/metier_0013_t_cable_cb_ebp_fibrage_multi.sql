...
{{ ctrl_specifique(
    ...
    requ_princ="SELECT "
                ...
                ~ "    JOIN ( "
                ~ "        SELECT ps_1 AS fo_code, ps_cs_code FROM " ~ source('gracethd', 't_position') ~ " WHERE ps_1 IS NOT NULL "
                ~ "        UNION ALL "
                ~ "        SELECT ps_2 AS fo_code, ps_cs_code FROM " ~ source('gracethd', 't_position') ~ " WHERE ps_2 IS NOT NULL "
                ~ "    ) p ON p.fo_code = f.fo_code "
