...
{{
    ctrl_specifique(
        ...
        condition      = "(src.lc_dateins IS NULL OR trim(src.lc_dateins::text) = '') AND src.lc_typelog IN ('SRO','NRO') AND src.lc_statut IN ('REC','MCO')",
        detail_erreur  = "'lc_typelog = ' || src.lc_typelog || ' / lc_statut = ' || src.lc_statut",
        is_active      = get_rc_config('ctrl_rc_0605')
    )
}}
